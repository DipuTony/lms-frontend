...
function CourseList(props) {
    ...
    const { api_getAllCouses, bearerHeader, baseUrl } = ApiList();
    ...
    const COLUMNS = [
        ...
        {
            Header: 'Image',
            accessor: 'image_full_path',
            Cell: ({ cell }) => (
                <div>
                    <div>  <div className=''> <img className='h-12 w-12 rounded-md' src={baseUrl + '/' + cell.row.values.image_full_path} alt="Course Image" />   </div> </div>
                </div>
            )
        },
        {
        ...
        },
        {
            ...
            Cell: ({ cell }) => (
                <div>
                    <div className=''>₹{cell.row.values.fee}</div>
                </div>
            )
        },
        {
            ...
            Cell: ({ cell }) => (
                <div>
                    <div className=''> {cell.row.values.discount}%  </div>
                </div>
            )
        },
        {
            ...
            Cell: ({ cell }) => (
                <div>
                    <div className=''>₹{cell.row.values.final_price}</div>
                </div>
            )
        },
        {
    ...
    const { isLoading, data, isError, error, refetch } = useQuery("api_getAllCouses", () => {
        ...
        <>
            ...
            <div>
                <div className='flow-root bg-white border-b-2 rounded-t-md md:px-5 p-2'>
                    <div className='float-left text-lg font-semibold flex'> <FcComboChart className='mt-1.5 mr-2' /> List of Courses</div>
